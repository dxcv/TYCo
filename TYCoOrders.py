...
class TYCoOrders:
    ...
    #! [bracket]
    @staticmethod
    def stopBracketOrder(parentOrderId: int, action: str, quantity: float,
                         stopPrice: float, takeProfitLimitPrice: float,
                         stopLossPrice: float):
   ...
   # ! [oca]
    ...
    def MarketFHedge(parentOrderId: int, action: str):
        ...
        order = TYCoOrders.MarketOrder(action, 0)
    ...
    def AttachAdjustableToStop(parent: Order, attachedOrderStopPrice: float,
        ...
        order = TYCoOrders.Stop("SELL" if parent.action == "BUY" else "BUY",

...
def get_VAA_allocations(method='0', T='1'):
    ...
    if method == '0':
        ...
        RiskonG4 = [ReturnList("VOO", VOO13612We), ReturnList(
            "VEA", VEA13612We), ReturnList("VWO", VWO13612We), ReturnList("BND", BND13612We)]
        Riskoff = [ReturnList("SHY", SHY13612We), ReturnList(
            "IEF", IEF13612We), ReturnList("LQD", LQD13612We)]
    elif method == '1':
        print('\n'" Choose month's month average price to calculate 13612W ")
        RiskonG4 = [ReturnList("VOO", VOO13612Wa), ReturnList(
            "VEA", VEA13612Wa), ReturnList("VWO", VWO13612Wa), ReturnList("BND", BND13612Wa)]
        Riskoff = [ReturnList("SHY", SHY13612Wa), ReturnList(
            "IEF", IEF13612Wa), ReturnList("LQD", LQD13612Wa)]
    ...
    print('\n', " Allocation for VAA is ", " VOO ", VAAVOO, " VEA ", VAAVEA, " VWO ",
          VAAVWO, " BND ", VAABND, " SHY ", VAASHY, " IEF ", VAAIEF, " LQD ", VAALQD, '\n')
...
def get_ADM_allocations(method='0'):
    ...
    print('\n', " Allocation for ADM is ", " VOO ",
          ADMVOO, " SCZ ", ADMSCZ, " TLT ", ADMTLT, '\n')
...
def get_GEM_allocations(method='0'):
    ...
    print('\n', " Allocation for GEM is ", " VOO ",
          GEMVOO, " VEU ", GEMVEU, " BND ", GEMBND, '\n')
...
def get_AAA_allocations(method='0', val20days='1'):
    ...
    AAAUniverse = [ReturnListwithClose("VOO", VOO126D, VOOph.close), ReturnListwithClose("EZU", EZU126D, EZUph.close), ReturnListwithClose("EWJ", EWJ126D, EWJph.close), ReturnListwithClose("EEM", EEM126D, EEMph.close), ReturnListwithClose("VNQ", VNQ126D, VNQph.close), ReturnListwithClose("RWX", RWX126D, RWXph.close), ReturnListwithClose("IEF", IEF126D, IEFph.close), ReturnListwithClose("TLT", TLT126D, TLTph.close), ReturnListwithClose("DBC", DBC126D, DBCph.close),
    ...
    for i in range(4):
        closeArray = close_to_percentage_change(
            np.array(AAAUniverse[i+1].close))
    ...
    if val20days == '1':
        ...
        # 20-day volatilities for five products
        stds = np.std(top5returns[:, :20], axis=1)
        print(stds)
        for row in range(5):
            for column in range(5):
                covMatrix[row][column] = (
                    covMatrix[row][column]*stds[row]*stds[column])
    ...
    print('\n', " Allocation for AAA is ", " VOO ", AAAVOO, " EZU ", AAAEZU, " EWJ ", AAAEWJ, " EEM ", AAAEEM,
          " VNQ ", AAAVNQ, " RWX ", AAARWX, " IEF ", AAAIEF, " TLT ", AAATLT, " DBC ", AAADBC, " GLD ", AAAGLD,   '\n')
